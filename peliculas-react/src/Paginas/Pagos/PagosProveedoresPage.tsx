// src/Paginas/Pagos/PagosProveedoresPage.tsx
// Página principal de Pagos a Proveedores (CxP)

import { useEffect, useState, useCallback } from "react";
import { Link } from "react-router-dom";
import { pagosProveedorApi, type CxpDocumentoListItem, type EstadoCxp } from "../../Servicios/pagosApi";

const ESTADOS_CXP: { value: EstadoCxp | ""; label: string }[] = [
...
export default function PagosProveedoresPage() {
  const [rows, setRows] = useState<CxpDocumentoListItem[]>([]);
  ...
  // Filtros
  const [estado, setEstado] = useState<EstadoCxp | "">("");
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await pagosProveedorApi.listarDocumentos({
    ...
    <div className="p-6">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Gestión de cuentas por pagar (CxP)</p>
        </div>
        <div className="flex gap-2">
          <Link
            to="/pagos/proveedores/generar"
            className="px-4 py-2 rounded-xl bg-sky-600 text-white hover:bg-sky-700 transition-colors"
          >
            Generar Consolidado
          </Link>
          <Link
            to="/pagos/proveedores/nuevo"
      ...
      {resumen && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-white rounded-2xl shadow p-4">
            ...
            <p className="text-xl font-bold text-gray-900">{formatCurrency(resumen.montoTotalDeuda)}</p>
          </div>
          <div className="bg-white rounded-2xl shadow p-4">
            <p className="text-sm text-gray-500">Total Pagado</p>
            <p className="text-xl font-bold text-green-600">{formatCurrency(resumen.montoTotalPagado)}</p>
          </div>
          <div className="bg-white rounded-2xl shadow p-4">
      ...
      <div className="bg-white rounded-2xl shadow p-4 mb-6">
        <div className="flex flex-wrap gap-4 items-end">
          <div>
            ...
            <select
              value={estado}
              onChange={(e) => setEstado(e.target.value as EstadoCxp | "")}
          ...
          <table className="min-w-full text-sm">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-3 text-left font-medium text-gray-600">Fact. Proveedor</th>
                ...
                rows.map((row) => (
                  <tr key={row.id} className="border-t hover:bg-gray-50">
                    <td className="px-4 py-3">
                      <Link
                        to={`/pagos/proveedores/documento/${row.id}`}
                    ...
                    <td className="px-4 py-3">
                      <div>
                        <p className="font-medium">{row.proveedorNombre}</p>
                        <p className="text-xs text-gray-500">{row.proveedorRnc}</p>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex justify-end gap-2">
                        {row.estadoNombre !== "Pagado" && row.estadoNombre !== "Anulado" && (
                          <Link
                            to={`/pagos/proveedores/pagar/${row.id}`}
