// src/Paginas/Pagos/ReporteHistoricoCobrosPage.tsx
// Reporte de histórico de cobros (CxC)
...
export default function ReporteHistoricoCobrosPage() {
  const [pagos, setPagos] = useState<Pago[]>([]);
  const [porDia, setPorDia] = useState<ResumenPorDia[]>([]);
  const [porMetodo, setPorMetodo] = useState<ResumenPorMetodo[]>([]);
  const [resumen, setResumen] = useState({ totalCobros: 0, montoTotal: 0 });
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const { data: result } = await api.get(`/reportes/pagos/historico-cobros?${params}`);
      setPagos(result.pagos || []);
      setPorDia(result.porDia || []);
      setPorMetodo(result.porMetodoPago || []);
      setResumen(result.resumen || { totalCobros: 0, montoTotal: 0 });
    } catch (err: unknown) {
    ...
    <div className="p-6">
      ...
      <div className="mb-6">
        <div className="flex items-center gap-2 text-gray-500 text-sm mb-2">
          ...
          <span>→</span>
          <span className="text-gray-900">Histórico de Cobros</span>
        </div>
        <h1 className="text-2xl font-semibold text-gray-900">Histórico de Cobros</h1>
        <p className="text-sm text-gray-500 mt-1">Detalle de todos los cobros realizados</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-white rounded-2xl shadow p-4">
          ...
          <p className="text-3xl font-bold text-gray-900">{resumen.totalCobros}</p>
        </div>
        <div className="bg-green-50 rounded-2xl shadow p-4">
          <p className="text-sm text-green-600">Monto Total Cobrado</p>
          <p className="text-3xl font-bold text-green-700">{formatCurrency(resumen.montoTotal)}</p>
        </div>
        <div className="bg-sky-50 rounded-2xl shadow p-4">
          <p className="text-sm text-sky-600">Promedio por Cobro</p>
          <p className="text-3xl font-bold text-sky-700">
            {resumen.totalCobros > 0 ? formatCurrency(resumen.montoTotal / resumen.totalCobros) : "RD$0.00"}
          </p>
        </div>
      </div>
          ...
          <div className="flex flex-wrap gap-3">
            {porMetodo.map((m) => (
              <div key={m.metodoPago} className="bg-gray-50 rounded-xl px-4 py-2">
                ...
                <p className="text-xs text-gray-500">{m.cantidad} cobros</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow overflow-hidden">
        <div className="px-4 py-3 border-b bg-gray-50">
          <h2 className="font-semibold text-gray-900">Detalle de Cobros</h2>
        </div>
            ...
            <tbody className="divide-y divide-gray-100">
              {pagos.map((pago) => (
                <tr key={pago.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">{pago.empresaNombre}</td>
