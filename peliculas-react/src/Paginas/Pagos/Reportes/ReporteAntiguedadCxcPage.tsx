...
// src/Paginas/Pagos/Reportes/ReporteAntiguedadCxcPage.tsx
...
type ResumenRango = { rango: string; cantidad: number; montoPendiente: number };
...
export default function ReporteAntiguedadCxcPage() {
  ...
  const exportToCSV = () => {
    const headers = ["Documento","Empresa","Vencimiento","Días Vencido","Rango","Total","Pagado","Pendiente","Estado"];
    const rows = documentos.map(d => [
      d.numeroDocumento, d.empresaNombre, formatDate(d.fechaVencimiento),
      d.diasVencido.toString(), d.rangoAntiguedad,
      d.montoTotal.toFixed(2), d.montoPagado.toFixed(2), d.montoPendiente.toFixed(2), d.estadoNombre
    ]);
    const csv = [headers.join(","), ...rows.map(r => r.map(c => `"${c}"`).join(","))].join("\n");
    const blob = new Blob(["\ufeff" + csv], { type: "text/csv;charset=utf-8;" });
    const link = document.createElement("a");
    link.href = URL.createObjectURL(blob);
    link.download = `antiguedad_cxc_${new Date().toISOString().split("T")[0]}.csv`;
    link.click();
  };

  const handlePrint = useReactToPrint({ contentRef: printRef, documentTitle: "Antiguedad_CxC" });
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-3 mb-1 text-sm">
            ...
            <span className="text-gray-300">/</span>
            <span className="text-gray-900 font-medium">Antigüedad CxC</span>
          </div>
          <h1 className="text-2xl font-bold text-gray-900">📊 Antigüedad de Cuentas por Cobrar</h1>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border p-6">
        <div className="flex items-end gap-4">
          <div className="flex-1">
            ...
            <select value={empresaId} onChange={e => setEmpresaId(e.target.value)} className="w-full px-3 py-2 border rounded-lg">
      ...
      {resumen && (
        <div className="bg-gradient-to-r from-sky-500 to-sky-600 rounded-2xl p-6 text-white">
          <div className="grid grid-cols-2 gap-6">
            <div><p className="text-sky-100">Total Documentos</p><p className="text-3xl font-bold">{resumen.totalDocumentos}</p></div>
            <div><p className="text-sky-100">Total por Cobrar</p><p className="text-3xl font-bold">{formatCurrency(resumen.totalPendiente)}</p></div>
          </div>
        </div>
      )}
      ...
      <div ref={printRef} className="bg-white rounded-2xl shadow-sm border overflow-hidden">
        <div className="hidden print:block p-6 border-b text-center">
          <h1 className="text-xl font-bold">Antigüedad - Cuentas por Cobrar</h1>
        ...
        {loading ? (
          ...
          <table className="w-full">
              ...
              {documentos.map(d => (
                <tr key={d.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3 text-sm font-medium">
                    {d.numeroDocumento}
                    {d.refinanciado && <span className="ml-2 text-xs bg-purple-100 text-purple-700 px-2 py-0.5 rounded">Ref</span>}
                  </td>
                  <td className="px-4 py-3 text-sm">{d.empresaNombre}</td>
            ...
            <tfoot className="bg-gray-50">
              <tr>
                <td colSpan={5} className="px-4 py-3 font-semibold text-right">TOTAL:</td>
